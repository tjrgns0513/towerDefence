...
public class MapEditorWindow : EditorWindow
{
    ...
    private void HandleMouseClickOrDrag(Event e)
    {
        ...
        if (!plane.Raycast(ray, out float distnace)) return;

        Vector3 point = ray.GetPoint(distnace);
        point = SnapToGrid(point, snapValue);

        var x = Mathf.FloorToInt(point.x / mapManager.gridSize);
        var y = Mathf.FloorToInt(point.z / mapManager.gridSize);

        string undoInfo = ProcessMouseInput(point, x, y);
        ModifyData(undoInfo);

        e.Use();
    }

    private string ProcessMouseInput(Vector3 point, int x, int y)
    {
        if (addingWaypoint)
        {
            AddWaypoint(point, x, y);
            return $"Add Waypoint({x},{y})";
        }
        else if (addingCube)
        {
